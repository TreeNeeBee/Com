/**
 * @file        HelloWorld2ServiceServerApp.hpp
 * @author      Aii
 * @brief       Auto-generated server application framework for HelloWorld2Service
 * @date        2026/02/09
...
#ifndef HELLOWORLD2_HELLOWORLD2SERVICESERVERAPP_HPP
#define HELLOWORLD2_HELLOWORLD2SERVICESERVERAPP_HPP
...
/**
 * @brief Server application framework for HelloWorld2Service
 ...
 */
class HelloWorld2ServiceServerApp
{
public:
    virtual ~HelloWorld2ServiceServerApp() = default;

    // ==================== Method Handlers (implement business logic) ====================

    /// Handle SayHello → String
    virtual ::lap::core::Future< String > OnSayHello( String visitorName ) = 0;
    ...
    void SendGreeting( const String& text )
    {
        if ( !m_pSkeleton ) { return; }
        auto sample = m_pSkeleton->greeting.Allocate();
        if ( sample.HasValue() )
        {
            sample.Value()->text = text;
            m_pSkeleton->greeting.Send( ::std::move( sample ).Value() );
        }
    }
    ...
    void SendStatusChanged( const HelloWorld2Types::ServerStatus& status )
    {
        if ( !m_pSkeleton ) { return; }
        auto sample = m_pSkeleton->statusChanged.Allocate();
        if ( sample.HasValue() )
    ...
    void SendDataStream( const HelloWorld2Types::DataChunk& chunk )
    {
        if ( !m_pSkeleton ) { return; }
        auto sample = m_pSkeleton->dataStream.Allocate();
        if ( sample.HasValue() )
    ...
    /// Notify subscribers of VisitorCount change
    void UpdateVisitorCount( UInt32 value )
    {
        if ( m_pSkeleton ) { m_pSkeleton->visitorCount.Update( value ); }
    }

    /// Notify subscribers of ServerName change
    void UpdateServerName( const String& value )
    {
        if ( m_pSkeleton ) { m_pSkeleton->serverName.Update( value ); }
    }
    ...
    int Run( int /* argc */ = 0, char** /* argv */ = nullptr )
    {
        ...
        s_instance_ = this;
        ::std::signal( SIGINT, &HelloWorld2ServiceServerApp::signalHandler_ );
        ::std::signal( SIGTERM, &HelloWorld2ServiceServerApp::signalHandler_ );

        ::std::cout << "=== " << skeleton::HelloWorld2ServiceSkeleton::kServiceName << " Server (Dual-Binding) ===" << ::std::endl;
        ...
        if ( pDds )
        {
            dds_adapter::RegisterHelloWorld2ServiceDdsAdapters(
                skeleton::HelloWorld2ServiceSkeleton::kServiceId );
        }

        // Phase 6 — Create Skeleton
        skeleton::HelloWorld2ServiceSkeleton skel(
            ::lap::core::InstanceSpecifier( "HelloWorld2Service/Provider" ) );
        m_pSkeleton = &skel;

        // Phase 7 — Wire Method Handlers → virtual callbacks
        skel.sayHello.RegisterMethodHandler(
            [this]( String visitorName ) -> ::lap::core::Future< String >
            { return OnSayHello( ::std::move( visitorName ) ); } );
...
protected:
    ...
    skeleton::HelloWorld2ServiceSkeleton& GetSkeleton() { return *m_pSkeleton; }
...
private:
    ...
    static inline HelloWorld2ServiceServerApp* s_instance_ = nullptr;
    ::std::atomic< bool > m_running{ true };
    skeleton::HelloWorld2ServiceSkeleton* m_pSkeleton = nullptr;
}; // class HelloWorld2ServiceServerApp
...
#endif // HELLOWORLD2_HELLOWORLD2SERVICESERVERAPP_HPP
