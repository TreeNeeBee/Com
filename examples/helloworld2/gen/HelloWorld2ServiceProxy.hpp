/**
 * @file        HelloWorld2ServiceProxy.hpp
 * @author      Aii
 * @brief       Auto-generated service proxy for HelloWorld2Service [SWS_CM_00004]
 * @date        2026/02/09
...
#ifndef HELLOWORLD2_HELLOWORLD2SERVICEPROXY_HPP
#define HELLOWORLD2_HELLOWORLD2SERVICEPROXY_HPP
...
namespace fields
{
    ...
    /**
     * @brief Service proxy for HelloWorld2Service [SWS_CM_00004]
     * @note Auto-generated — non-copyable, move-only, named constructor
     * @version 1.0.0
     */
    class HelloWorld2ServiceProxy final : public ::lap::com::ProxyBase {
    public:
        using HandleType = ::lap::com::ServiceHandleType< HelloWorld2ServiceProxy >;

        // ==================== Service Identification ====================
        static constexpr UInt16 kServiceId = 0x0278;
         ...
         */
        static Result< HelloWorld2ServiceProxy > Create( const HandleType& handle ) noexcept {
            if ( !handle.IsValid() ) {
                return Result< HelloWorld2ServiceProxy >::FromError(
                    MakeErrorCode( ComErrc::kServiceNotAvailable, 0 ) );
            }

            HelloWorld2ServiceProxy proxy( handle );
            ...
            return Result< HelloWorld2ServiceProxy >::FromValue( ::std::move( proxy ) );
        }

        ~HelloWorld2ServiceProxy() noexcept override = default;

        // Move-only [SWS_CM_11554, SWS_CM_11552]
        HelloWorld2ServiceProxy( HelloWorld2ServiceProxy&& ) noexcept = default;
        HelloWorld2ServiceProxy& operator=( HelloWorld2ServiceProxy&& ) noexcept = default;

        // Non-copyable [SWS_CM_11553, SWS_CM_11551]
        HelloWorld2ServiceProxy( const HelloWorld2ServiceProxy& ) = delete;
        HelloWorld2ServiceProxy& operator=( const HelloWorld2ServiceProxy& ) = delete;
        ...
        // ==================== Fields [SWS_CM_99446] ====================
        fields::VisitorCount visitorCount{ true, false, true }; ///< @note readonly
        fields::ServerName serverName{ true, true, true };
    ...
    protected:
         ...
         */
        explicit HelloWorld2ServiceProxy( const HandleType& handle ) noexcept
            : ::lap::com::ProxyBase()
            , m_handle( handle )
        {}
...
#endif // HELLOWORLD2_HELLOWORLD2SERVICEPROXY_HPP
