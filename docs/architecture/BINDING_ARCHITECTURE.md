...
- [6. 序列化策略 (IDL 驱动)](#6-序列化策略-符合-autosar-标准)
...
| 传输层 | IDL | 序列化路径 | 输出格式 |
|-------|-----|-----------|--------|
| DDS（强类型） | Franca IDL (.fidl) → OMG IDL (.idl) | `lap-sidl-gen --server/--client` → fastddsgen → `IDdsTypeAdapter` | FastDDS 自动 CDR |
| DDS（后备） | — | `DdsPayloadPubSubType` 手写 CDR 特化 | `DdsPayload { requestId, data[] }` |
| CoreIPC | — | `CBinarySerializer` (ADL 自由函数) | 二进制 (最小开销) |
| SOME/IP ⚠️待实现 | Franca IDL (.fidl) | `lap-sidl-gen` → SOME/IP 序列化器 | SOME/IP wire format |
