...
### 1.4 架构层次（IPC-based Registry Service v2.0）
...
│  │  - FindService() → 本地读取共享内存槽位 (< 500ns，零IPC)            │ │
│  │  - OfferService() → IPC请求到Registry Service (< 50μs)              │ │
│  └─────────────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────────────┘
                          ↓ (标准 ara::com API 调用)
┌──────────────────────────────────────────────────────────────────────────┐
│          lap::com Runtime (客户端轻量级，~500 行代码)                      │
│  ┌──────────────────────────────────────────────────────────────────────┐│
│  │ RegistryClient (IPC客户端)                                            ││
│  │  ┌────────────────────────────────────────────────────────────────┐  ││
│  │  │ 只读共享内存映射 (mmap with PROT_READ)                         │  ││
│  │  │  - QM Registry: 256KB (1024 slots × 256 bytes)                 │  ││
│  │  │  - ASIL Registry: 256KB (1024 slots × 256 bytes)               │  ││
│  │  │  - FindService: 直接读取，零IPC开销                            │  ││
│  │  └────────────────────────────────────────────────────────────────┘  ││
│  │  ┌────────────────────────────────────────────────────────────────┐  ││
│  │  │ Core IPC Publisher (/lap_registry_req)                         │  ││
│  │  │  - 发送RegistryRequest (MPSC通道)                              │  ││
│  │  │  - REGISTER_SERVICE / UNREGISTER_SERVICE / UPDATE_HEARTBEAT    │  ││
│  │  │  - request_id生成 (correlation ID)                             │  ││
│  │  └────────────────────────────────────────────────────────────────┘  ││
│  │  ┌────────────────────────────────────────────────────────────────┐  ││
│  │  │ Core IPC Subscriber (/lap_registry_resp)                       │  ││
│  │  │  - 接收RegistryResponse (SPMC通道)                             │  ││
│  │  │  - 通过request_id匹配响应                                      │  ││
│  │  │  - 后台线程监听 + 条件变量通知                                 │  ││
│  │  └────────────────────────────────────────────────────────────────┘  ││
│  └──────────────────────────────────────────────────────────────────────┘│
└──────────────────────────────────────────────────────────────────────────┘
                          ↓ (Core IPC 通道)
┌──────────────────────────────────────────────────────────────────────────┐
│          Registry Service (中心化服务，单独进程)                           │
│  ┌──────────────────────────────────────────────────────────────────────┐│
│  │ RegistryService (IPC事件循环 + 单写者模型)                            ││
│  │  ┌────────────────────────────────────────────────────────────────┐  ││
│  │  │ 可写共享内存 (独占写权限)                                      │  ││
│  │  │  - QM Registry: mmap with PROT_READ | PROT_WRITE              │  ││
│  │  │  - ASIL Registry: mmap with PROT_READ | PROT_WRITE            │  ││
│  │  │  - 单写者保证：所有槽位修改在单线程串行执行                    │  ││
│  │  └────────────────────────────────────────────────────────────────┘  ││
│  │  ┌────────────────────────────────────────────────────────────────┐  ││
│  │  │ Core IPC Subscriber (/lap_registry_req)                        │  ││
│  │  │  - 接收RegistryRequest (MPSC通道，阻塞接收)                    │  ││
│  │  │  - 事件循环：Receive → Process → Send Response                │  ││
│  │  │  - 请求处理：注册/注销/心跳更新                                │  ││
│  │  └────────────────────────────────────────────────────────────────┘  ││
│  │  ┌────────────────────────────────────────────────────────────────┐  ││
│  │  │ Core IPC Publisher (/lap_registry_resp)                        │  ││
│  │  │  - 广播RegistryResponse (SPMC通道)                             │  ││
│  │  │  - 所有客户端接收，通过request_id自行过滤                      │  ││
│  │  │  - 成功/失败状态 + 错误消息                                    │  ││
│  │  └────────────────────────────────────────────────────────────────┘  ││
│  │  ┌────────────────────────────────────────────────────────────────┐  ││
│  │  │ 心跳监控线程 (可选)                                            │  ││
...
| 特性 | 传统方案 | v3.0 (直接写入) | **v2.0 (IPC-based)** |
|------|----------|-----------------|----------------------|
| **FindService延迟** | 1-5µs (Domain Socket) | < 500ns | **< 500ns** (本地读取) |
| **OfferService延迟** | 1-5µs (Domain Socket) | < 1µs (直接写入) | **< 50µs** (IPC往返) |
| **守护进程** | RouDi / systemd-resolved | ❌ 零 Daemon | **Registry Service** (轻量) |
| **单点故障** | ✅ 存在 | ❌ 无 | **⚠️ Registry Service** (可容错) |
| **确定性** | 中 (网络抖动) | ✅ 高 | **✅ 高** (Core IPC确定性) |
| **一致性保证** | 弱 (竞态条件) | 弱 (多写者竞争) | **✅ 强** (单写者原子操作) |
| **启动依赖** | 需先启动 Daemon | ✅ 无依赖 | **Registry Service先启动** |
| **FuSa 友好** | 需隔离 Daemon | ✅ 天然隔离 | **✅✅ 更好** (权限集中管控) |
| **代码复杂度** | 高 (>2000 行) | 低 (~300 行) | **中** (~800行，客户端500+服务300) |
| **可审计性** | 低 | 无 | **✅ 高** (所有操作可追踪) |
| **安全性** | 中 | 中 (直接写入风险) | **✅ 高** (统一权限验证) |
...
✅ **可审计**: 所有修改操作可记录追踪  
✅ **安全性**: 统一权限检查和访问控制  
✅ **可靠性**: Registry Service故障可快速重启恢复  
⚠️ **延迟增加**: OfferService从<1µs增至<50µs (可接受)  
⚠️ **单点依赖**: Registry Service需先启动 (systemd保证)
...
## 第 2 章: IPC-based Registry Service 架构 (v2.0)

### 2.0 架构演进说明

**v1.0 → v2.0 重大变更**：

从**完全去中心化**（多写者竞争）转向**中心化服务**（单写者原子操作）。

#### 2.0.1 设计动机

v1.0架构（直接写入共享内存）存在以下问题：

1. **竞态条件**: 多进程并发写入同一槽位，seqlock只保证读原子性，无法防止写冲突
2. **权限管控**: 所有进程都有写权限（0666），无法实施细粒度访问控制
3. **可审计性**: 无法追踪谁注册/注销了服务，调试困难
4. **FuSa合规性**: ISO 26262要求关键服务修改必须可审计和可回溯

#### 2.0.2 IPC-based架构核心思想

```text
核心原则: 读写分离
- FindService:  本地共享内存读取 (< 500ns，零IPC开销)
- OfferService: IPC请求到Registry Service (< 50µs，可接受延迟)

单写者模型:
- Registry Service独占写权限
- 所有槽位修改串行化执行
- 消除竞态条件和数据损坏风险
```
...
**MPSC (Multi-Publisher Single-Consumer) 请求通道**:
- 路径: `/lap_registry_req`
- 方向: 多个客户端 → Registry Service
- 消息: `RegistryRequest` (160字节)
- 操作类型: REGISTER_SERVICE / UNREGISTER_SERVICE / UPDATE_HEARTBEAT

**SPMC (Single-Publisher Multi-Consumer) 响应通道**:
- 路径: `/lap_registry_resp`
- 方向: Registry Service → 多个客户端
- 消息: `RegistryResponse` (96字节)
- 匹配机制: 通过`request_id` (correlation ID)

#### 2.0.4 请求-响应流程

```text
客户端                     Registry Service
  │                              │
  ├─① 生成request_id             │
  ├─② 构造RegistryRequest        │
  ├─③ 发送到MPSC通道─────────────>│
...
**v1.0** (多写者):
```c
QM Registry:   mmap(PROT_READ | PROT_WRITE)  // 所有进程可写
ASIL Registry: mmap(PROT_READ | PROT_WRITE)  // 所有进程可写
问题: 权限过于宽松，无法防止恶意/意外篡改
```

**v2.0** (单写者):
```c
Registry Service:
  QM Registry:   mmap(PROT_READ | PROT_WRITE)  // 独占写权限
  ASIL Registry: mmap(PROT_READ | PROT_WRITE)  // 独占写权限

Clients:
  QM Registry:   mmap(PROT_READ)               // 只读映射
  ASIL Registry: mmap(PROT_READ)               // 只读映射
```

#### 2.0.6 故障恢复机制

**Registry Service崩溃场景**:

1. **启动时**: systemd自动重启Registry Service (Restart=always)
2. **恢复过程**:
   - 重新映射共享内存 (数据持久化在memfd中，未丢失)
   - 扫描所有槽位，验证进程存活性 (kill(pid, 0))
   - 清理僵尸槽位 (进程已退出但槽位未清理)
   - 恢复IPC通道监听
3. **客户端处理**:
   - IPC请求超时 (5秒)
   - 自动重试机制 (指数退避)
   - 降级策略: 只读模式继续提供FindService

**性能影响**:
- Registry Service重启时间: < 100ms
- 客户端感知延迟: 首次请求超时 + 重试 ≈ 5-10秒
- 缓解措施: systemd watchdog + 健康检查端点

---

## 第 2 章 (续): 零 Daemon 固定槽位自注册架构
...
**双层 IDL 架构**: Franca IDL 作为 SSOT，通过 `lap-sidl-gen` 驱动 AUTOSAR API 和 DDS IDL 双层代码生成。
架构图和详细设计见 [GENERATOR.md §11 双层 IDL 设计](GENERATOR.md#11-双层-idl-设计)。

**8 大一致性保证**:

##### ✅ (1) Franca IDL 作为单一真相源 (SSOT)

所有服务接口通过 Franca IDL 定义，由 `lap-sidl-gen`（纯 C++ 实现）统一生成 AUTOSAR API 和 DDS IDL。

**关键机制**:
- Franca IDL 定义服务契约（版本 + 类型 + 接口）
- `lap-sidl-gen` 自动计算 Schema Hash（FNV-1a 64-bit → 16 字符 hex）
- 同时生成 AUTOSAR C++ API（Skeleton/Proxy）和 DDS IDL v4.2
- Franca 类型自动映射为 DDS IDL 类型（如 `UInt32` → `unsigned long`）

> 完整的 IDL 示例、类型映射表、工作流脚本和 DDS IDL 生成示例详见 [GENERATOR.md §4.6, §10.6-10.7, §12](GENERATOR.md)。
...
#include "RadarServiceProxy.h"  // AUTOSAR API (lap-sidl-gen 生成)
...
**CI 阶段版本检查**: 在 CI pipeline 中使用 `lap-sidl-gen --validate` 结合 ECU manifest 文件检查跨 ECU 服务版本一致性。
详见 [GENERATOR.md §4 CLI 使用](GENERATOR.md#4-cli-使用)。
...
**版本检查工具**: 通过 `lap-sidl-gen --hash-only` + YAML manifest 比对实现跨 ECU 版本一致性检查。

> 工具链使用详见 [GENERATOR.md §4 CLI 使用](GENERATOR.md#4-cli-使用)。
...
**Service Contract Repository 结构**: 所有服务的 Franca IDL、版本策略、ECU manifest 和公共类型集中管理在 Git Repository 中。`generated/` 目录由 `lap-sidl-gen` 和 FastDDS-gen 自动生成（不提交到 Git）。

> 目录结构和生成工具详见 [GENERATOR.md §2 目录结构](GENERATOR.md#2-目录结构) 和 [§4.6 生产环境工作流](GENERATOR.md#46-生产环境-5-步工作流)。

**ECU 构建时依赖**: 通过 CMake `FetchContent` 拉取 Service Contract Repository，使用 `lap-sidl-gen` 完成 Schema Hash 生成 → AUTOSAR API 生成 → DDS IDL 转换 → FastDDS-gen TypeSupport 的 5 步流程。应用程序只链接 AUTOSAR API 库，不直接依赖 DDS。

> 完整 CMake 集成示例详见 [GENERATOR.md §4.6 生产环境工作流](GENERATOR.md#46-生产环境-5-步工作流)。
...
| 阶段 | 内容 | 工具 |
|------|------|------|
| validate | Franca IDL 语法验证 + Schema Hash 生成 | `lap-sidl-gen --validate`, `--hash-only` |
| generate | AUTOSAR API + DDS IDL 生成 + Hash 一致性校验 | `lap-sidl-gen`, `fastddsgen` |
| build | 所有 ECU 并行构建 + 二进制 Schema Hash 嵌入验证 | CMake |
| test | 跨 ECU 运行时 SD-Proxy Schema 验证 | Docker + `journalctl` |

> validate/generate 阶段详细脚本见 [GENERATOR.md §4.6 生产环境工作流](GENERATOR.md#46-生产环境-5-步工作流)。

**Schema Hash 一致性验证**: 通过 `lap-sidl-gen --hash-only` 计算 Franca Hash，与 DDS IDL 中嵌入的 Hash 比对，确保端到端一致性。

> 详见 [GENERATOR.md §9 Schema Hash](GENERATOR.md#9-schema-hash-cschemahash)。

**版本策略检查**: 使用 `lap-sidl-gen --validate` 检查 Franca IDL 语法完整性、版本声明和 interface 定义合规性。

> 详见 [GENERATOR.md §4.2 生成模式](GENERATOR.md#42-生成模式)。
...
双层 IDL 架构（Franca IDL + DDS IDL）相比传统单层 DDS IDL 在 11 个方面具有显著优势：
平台无关性、Transport 抽象、编译隔离、版本管理、一致性验证等。
...
> 完整的架构对比表、开发流程对比图和核心价值分析详见 [GENERATOR.md §11 双层 IDL 设计](GENERATOR.md#11-双层-idl-设计)。
...
- ✅ 单元测试 (覆盖率 >80%)
- ✅ **Franca IDL 工具链搭建**
  - ✅ lap-sidl-gen 纯 C++ 实现 (取代 PyFranca)
  - ✅ Schema Hash 生成 (--hash-only)
  - ✅ Franca → DDS IDL 转换 (--dds-idl)
...
- ✅ **双层 IDL 代码生成集成**
  - ✅ lap-sidl-gen 生成 AUTOSAR API (Skeleton/Proxy)
...
- 📋 **CI/CD Pipeline**
  - Franca IDL 验证 (lap-sidl-gen --validate)
  - Schema Hash 一致性检查
  - 版本策略合规性验证
  - 自动代码生成和测试
  - 跨 ECU 兼容性测试
...
### 12.1 服务发现延迟对比（v2.0 IPC-based架构）

| 发现方式 | 平均延迟 | P99 延迟 | 适用场景 |
|----------|----------|----------|----------|
| **固定槽位查找 (FindService)** | < 100 ns | < 500 ns | 本地服务发现（零IPC开销）⭐ |
...
✅ **FindService保持零IPC开销**: 客户端直接读取共享内存（< 500ns）  
⚠️ **OfferService增加IPC延迟**: 从<1µs增至<50µs（可接受，非关键路径）  
✅ **心跳更新优化**: Fire-and-forget模式，仅单向发送（<20µs）  
✅ **整体吞吐**: Registry Service可支持 > 20,000 ops/s 注册请求

**延迟分解 (OfferService v2.0)**:

```text
客户端OfferService调用 (总计: ~30µs)
├─ 构造RegistryRequest消息          : 0.5 µs
├─ Core IPC Publisher::Send          : 2-5 µs
├─ Registry Service接收并处理         : 3-8 µs
│  ├─ 槽位写入 (seqlock + memcpy)    : 1 µs
│  └─ 构造RegistryResponse           : 0.5 µs
├─ Core IPC Publisher::Send (响应)   : 2-5 µs
├─ 客户端Subscriber接收              : 2-5 µs
└─ request_id匹配 + 条件变量通知      : 1-3 µs
```
...
**Registry Service进程**:
- 内存: ~512 KB (双注册表 + IPC Channels + 事件循环)
  - QM Registry: 256 KB (1024 slots × 256 bytes)
  - ASIL Registry: 256 KB (1024 slots × 256 bytes)
  - Core IPC Subscriber Buffer: ~256 KB (1024 requests × 160 bytes)
  - Core IPC Publisher Buffer: ~128 KB (128 responses × 96 bytes)
- CPU: < 1% (空闲)，< 5% (峰值，批量注册)
- 线程: 1主线程 (IPC事件循环) + 1可选心跳监控线程
- 依赖: Core IPC库，无其他外部依赖
- 端口: 无（共享内存访问）

**客户端进程开销**:
- 内存: ~400 KB per client
  - 只读共享内存映射: 512 KB (QM + ASIL)
  - Core IPC Publisher Buffer: ~64 KB (16 requests × 160 bytes)
  - Core IPC Subscriber Buffer: ~64 KB (256 responses × 96 bytes)
  - 响应监听线程栈: ~8 KB
- CPU: < 0.5% (空闲)，< 2% (响应监听线程)
- 线程: 1后台响应监听线程
- 共享内存访问: mmap PROT_READ (只读)，零拷贝

**对比v1.0 (直接写入)**:
- v1.0客户端内存: ~300 KB (少100KB，无IPC通道)
- v2.0总内存开销: +512KB (Registry Service) + 客户端每个+100KB
- 权衡: 内存增加 < 1MB，换取强一致性和可审计性

### 12.3 吞吐量

| 操作 | v1.0 (直接写入) | v2.0 (IPC-based) | 说明 |
|------|----------------|------------------|------|
| RegisterService | 100,000+ ops/s | **20,000+ ops/s** | 单Registry Service瓶颈 |
| FindService (本地) | 200,000+ ops/s | **200,000+ ops/s** | 零IPC，无变化 |
| FindService (注册中心) | 100,000+ ops/s | 100,000+ ops/s | 仅影响远程查询 |
| 并发查询 (4 线程) | 300,000+ ops/s | **300,000+ ops/s** | 读并发无锁竞争 |
| UpdateHeartbeat | 50,000+ ops/s | **40,000+ ops/s** | Fire-and-forget优化 |

**吞吐瓶颈分析 (v2.0)**:

- Registry Service单线程处理: 20,000 ops/s (平均50µs/op)
- 峰值负载: 启动时批量注册 (假设100个服务同时启动)
  - 总延迟: 100 × 50µs = 5ms (完全可接受)
- 稳态负载: 心跳更新 (1Hz per service, 1000 services)
  - 负载: 1000 ops/s (仅占5%容量，轻松应对)

### 12.4 性能对比总结 (v1.0 vs v2.0)

| 指标 | v1.0 (多写者) | v2.0 (单写者) | 评价 |
|------|--------------|--------------|------|
| FindService延迟 | < 500ns | **< 500ns** | ✅ 无变化 |
| OfferService延迟 | < 1µs | **< 50µs** | ⚠️ +50倍，但仍远低于DDS/SOME-IP |
| 注册吞吐 | 100K ops/s | **20K ops/s** | ⚠️ -80%，但实际负载<1K ops/s |
| 查询吞吐 | 200K ops/s | **200K ops/s** | ✅ 无变化 |
| 一致性保证 | 弱 (竞态) | **强 (原子)** | ✅ 质的提升 |
| 可审计性 | 无 | **完整** | ✅ 新增能力 |
| 安全性 | 中 | **高** | ✅ 统一权限管控 |
| 内存开销 | ~300KB | **~400KB** | ⚠️ +33%，可接受 |

**结论**: v2.0在极小的性能和资源代价下，换取了**强一致性、可审计性和安全性**的巨大提升，符合FuSa和生产环境需求。
...
`lap-sidl-gen`（纯 C++ 实现）提供完整的 Franca IDL 工具链，支持 6 种 CLI 模式：
验证、Schema Hash 生成、AUTOSAR API 生成、DDS IDL 转换、外部 Hash 注入、版本覆盖。

QoS 配置独立于 IDL，放在单独的 YAML 文件中（`qos_config/` 目录下）。

> 详细的 CLI 命令参考、模式对比表和工作流示例详见 [GENERATOR.md §4 命令行接口](GENERATOR.md#4-命令行接口-cli)。
...
- **lap-sidl-gen**: Franca IDL 解析和代码生成 (纯 C++ 实现，位于 `modules/Com/generator/`)
...
- **Franca IDL 工具链**: `modules/Com/generator/` (统一纯 C++ 实现)
  - `lap-sidl-gen --validate`: IDL 语法验证
  - `lap-sidl-gen --hash-only`: Schema Hash 生成 (FNV-1a 64-bit)
  - `lap-sidl-gen --proxy --skeleton`: AUTOSAR C++ API 生成
  - `lap-sidl-gen --dds-idl`: Franca → DDS IDL 转换
  - `tools/generate_all.sh`: 批量代码生成脚本 (调用 lap-sidl-gen)
