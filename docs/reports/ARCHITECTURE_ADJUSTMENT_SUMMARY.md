...
1. **移除 Protobuf + Domain Socket Binding**
   - 原因: CoreIPC 提供更优异的性能
   - 延迟对比: CoreIPC (<1μs) vs Protobuf+Socket (<5μs)
   - 吞吐量对比: CoreIPC (>10GB/s) vs Protobuf+Socket (>1GB/s)
   - 设计已归档至: `archive/PROTOBUF_SOCKET_BINDING_ARCHIVED.md`

2. **新增 CoreIPC Binding**
...
| **CoreIPC** | **<1μs** | **>10,000 MB/s** | **✅✅** | **✅✅** |

### 技术栈简化

- **减少依赖**: 移除Protobuf库依赖
- **统一方案**: 本地高性能IPC统一使用CoreIPC
...
2. **Phase 2 (中优先级)**: CoreIPC Binding实现
   - 预计: 5周
   - 代码量: ~2,200行
...
**归档原因**: CoreIPC提供10倍以上的性能优势，覆盖所有Protobuf+Socket的使用场景。
